require "spec_helper"
require "generators/mono/install/install_generator"

describe Mono::Generators::InstallGenerator, :generator do
 

  describe "application controller" do
    it "includes Mono::Controller" do
      ...
      expect(application_controller).to contain("include Mono::Controller")
    end
  end

  describe "user_model" do
    context "no existing user class" do
      it "creates a user class including Mono::User" do
        provide_existing_application_controller
        ...
        expect(user_class).to contain("include Mono::User")
      end
    end

    context "user class already exists" do
      it "includes Mono::User" do
        provide_existing_application_controller
  ...
  describe "user migration" do
    ...
    context "existing users table with all mono columns and indexes" do
      it "does not create a migration" do
        provide_existing_application_controller

        run_generator
        create_migration = migration_file("db/migrate/create_users.rb")
        add_migration = migration_file("db/migrate/add_mono_to_users.rb")
    ...
    context "existing users table missing some columns and indexes" do
      it "create a migration to add missing columns and indexes" do
        ...
        run_generator
        migration = migration_file("db/migrate/add_mono_to_users.rb")
